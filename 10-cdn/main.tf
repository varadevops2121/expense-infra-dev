...
resource "aws_cloudfront_distribution" "web_cdn" {
  origin {
    domain_name              =  "web-${var.environment}.${var.zone_name}"#web-dev.lerndevops.online
  ...
  aliases = ["web-${var.common_tags.Component}.${var.zone_name}"]#web-cdn.lerndevops.online
...
module "records" {
  ...
  records = [
    {
      name    = "web-cdn" #web-cdn.lerndevops.online
      type    = "A"
